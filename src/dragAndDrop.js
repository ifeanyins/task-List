...
function dragAndDropEvent (){
    ...
    project.addEventListener("mousedown", enableDraggable); // to enable only the three dots to drag not the whole tile
...
// callback function when you drag prjectTile over tiles
function dragOver(e){
    e.preventDefault();
    const project = document.querySelector(".project");
...
function getDragAfterElement(y){
    ...
    return projectTiles.reduce((closest, tile) =>{
        const box = tile.getBoundingClientRect(); //get the size of our tile like x y top bottom etc
        const offset = y - box.top - box.height/2; //offset betweeen the cursor and that tile center point

        // we want offset neg because the cursor would be above the center of that tile,
        // and compare that tot the latest closest.offset(first set to neg infinity)
        if(offset <0 && offset>closest.offset){
            return{
                offset: offset,
                element: tile //if it is the closest tile to the cursor return that tile amd its offset for comparing later
            }
        }
        else {
            return closest; // if this tile is not the closest then return the previous closest tile;
        }
